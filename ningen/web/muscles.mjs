...
export class MuscleDriver {
  constructor(mj, m, { kp = 120, kd = 22, reg = 2e-3, reserve = 8, rootForce = 3000, rootTorque = 1500, iters = 30, maxErr = Infinity, rootCarried = false, instant = false, bigReserve = reserve, stance = null, balance = null } = {}) {
    ...
    this.stance = stance;
    // balance: { ang: [kp, kd], z: [kp, kd], xy: [kp, kd], ahead } - with the root not carried, the root's
    // tilt, height and place are brought back toward their targets through the stance forces too (the
    // body leaning on its limbs). `ahead` (s) answers where the root will be that much later at its
    // present speed rather than where it is: pushing against a fall before it has got going.
    this.balance = balance;
    this.reg = 0.02;                          // how much the stance forces are kept small (and spread out)
    this.mu = 0.8;                            // how hard a support may push sideways, for how hard it pushes down
  ...
  step(d, qRef, qvelRef = null) {
    ...
    if (this.balance) {
      const { ang, z, xy, ahead = 0 } = this.balance;
      const pd = (g, k) => g[0] * (err[k] - ahead * qvel[k]) - g[1] * qvel[k];
      for (let k = 0; k < 2; k++) qacc[k] = xy ? pd(xy, k) : 0;
      qacc[2] = pd(z, 2);
      for (let k = 3; k < 6; k++) qacc[k] = pd(ang, k);
    }
    const qpos = d.qpos;
  ...
  shareOut(d, tau) {
    ...
    // (sideways forces cost more than vertical ones; a support about to be lifted - weight w below 1 -
    // costs more, so the others take its load before it goes)
    for (let x = 0; x < N; x++) A[x][x] += this.reg * (x % 3 === 2 ? 1 : 5) / Math.max(0.02, pts[Math.floor(x / 3)].w ?? 1);
    const f = gauss(A, b);
    let fz = 0;
    for (let i = 0; i < n; i++) {
      if (f[3 * i + 2] < 0) f[3 * i + 2] = 0;                                  // the floor only pushes
      const lim = this.mu * f[3 * i + 2];                                      // and holds sideways only so hard (friction)
  ...
  /** Joint stiffness at `scale` times its built-in value, and `big` N m of reserve on the big joints. */
  tune({ stiffness, big } = {}) {
    if (!this.ktBase) { this.ktBase = this.kt.slice(); this.kdtBase = this.kdt.slice(); this.resBase = this.reserveOf.slice(); this.bigBase = this.bigReserve; }
    if (stiffness != null) for (let k = 0; k < this.nv; k++) { this.kt[k] = this.ktBase[k] * stiffness; this.kdt[k] = this.kdtBase[k] * Math.sqrt(stiffness); }
    if (big != null) for (let k = 0; k < this.nv; k++) this.reserveOf[k] = this.resBase[k] === this.bigBase ? big : this.resBase[k] === 3 * this.bigBase ? 3 * big : this.resBase[k];
  }
