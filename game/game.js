// うんち陣取り — two players share one screen, place droppings, and the flies decide the winner.
//
...
class Fly {
  ...
  step(dt) {
    ...
    else if (this.state === 'walk' && this.pauseT > 0 && Math.random() < dt / 2) this.rub();
    ...
    if (this.state === 'walk' && !this.leaving && world.t - this.lastFlightEnd > 6 && Math.random() < dt / 45) this.takeoff('voluntary', this.yaw + rnd(-0.4, 0.4), 'body');
  }
...
function dropPoop(x, y, owner) {
  const p = { x, y, owner, z: 60, born: world.t, score: 0, count: 0, ...makeLumps(), anim: { phase: 'fall', t: 0, v: 0 } };
  poops.push(p);
  buildPoopMesh(p);
...
const cam = { az: -2.0, el: 0.78, dist: 72, target: new THREE.Vector3(0, 0, 1.5) };
...
view.addEventListener('pointerup', (e) => {
  ...
  else if (down && !down.orbit && !down.hand && down.moved <= 10 && performance.now() - down.t < 600) tapGround(e.clientX, e.clientY);
  if (down && down.hand) liftHand();
...
const game = { phase: 'intro', turn: 0, placed: [0, 0], left: WATCH_TIME, score: [0, 0], count: [0, 0] };
const groundPlane = new THREE.Plane(new THREE.Vector3(0, 0, 1), 0);

function startGame() {
  clearPoops();
  Object.assign(game, { phase: 'place', turn: 0, placed: [0, 0], left: WATCH_TIME, score: [0, 0], count: [0, 0] });
...
function placeAt(x, y) {
  ...
  addMarker(dropPoop(x, y, game.turn));
...
function startWatch() {
  game.phase = 'watch'; game.left = WATCH_TIME; game.endAt = performance.now() / 1000 + WATCH_TIME; game.score = [0, 0];
  for (const f of flies) {
...
// how many flies sit on each dropping, and the running score (fly-seconds)
function scoreTick(dt) {
  for (const p of poops) p.count = 0;
  for (const f of flies) {
    if (f.airborne || f.leaving || f.state === 'held') continue;
    let best = null, bd = 1e9;
    for (const p of poops) {
      if (p.z > 0.01) continue;
  ...
  if (game.phase === 'watch') {
    game.score[0] += game.count[0] * dt; game.score[1] += game.count[1] * dt;
...
function endGame() {
  ...
  const [a, b] = game.score.map(Math.round), win = a === b ? -1 : a > b ? 0 : 1;
  $('r-title').textContent = win < 0 ? '引き分け' : `${PLAYERS[win].name}の勝ち`;
  $('r-title').style.color = win < 0 ? '' : PLAYERS[win].css;
  $('r-score').innerHTML = `<span style="color:${PLAYERS[0].css}">赤 ${a}</span> <span class="vs">—</span> <span style="color:${PLAYERS[1].css}">青 ${b}</span>`;
  $('result').hidden = false;
}
function hud() {
  $('s0').textContent = Math.round(game.score[0]); $('s1').textContent = Math.round(game.score[1]);
  $('c0').textContent = game.count[0]; $('c1').textContent = game.count[1];
  const t = $('turn');
  if (game.phase === 'place') t.innerHTML = `<b style="color:${PLAYERS[game.turn].css}">${PLAYERS[game.turn].name}</b> の番 — 地面をタップしてうんちを置く（あと ${POOPS_EACH - game.placed[game.turn]} 個）`;
  else if (game.phase === 'watch') t.innerHTML = `のこり <b>${Math.ceil(game.left)}</b> 秒 — 集まっている数が毎秒スコアに`;
  else if (game.phase === 'result') t.textContent = '終了';
  else t.textContent = '';
...
function frame(now) {
  ...
  if (J) {                                 // body data loaded (flies may still be on their way)
    ...
    while (acc >= H) {
      ...
      if (Math.round(world.t * 1000) % 20 === 0) for (const f of flies) f.decideRule(0.02);
    ...
    stepPoops(dtReal);
    if ((scoreAcc += wall) >= 0.2) { scoreTick(scoreAcc); scoreAcc = 0; }
...
window.__game = { world, flies, cam, stats, renderer, audio, game, poops, startGame, placeAt, endGame, project: (f) => new THREE.Vector3(f.x, f.y, f.z).project(camera).toArray() };
requestAnimationFrame(frame);
